"""Bounded deterministic parallel Phase 9 action generation.

Workers operate on independent samples and the final result is sorted by stable
identities, so completion order cannot change scientific outputs.
"""
...
from .identities import action_engine_id, action_operational_config_id, action_schema_id
...
def build_action_engine_result_parallel(
    ...
    *,
    ...
    """Generate and rank Phase 9 actions using bounded shared-memory workers."""
    ...
    if isinstance(worker_count, bool) or not isinstance(worker_count, int):
        raise TypeError("workers must be an integer and not bool")
    if worker_count <= 0:
        raise ValueError("workers must be positive")
    if isinstance(progress_every, bool) or not isinstance(progress_every, int):
        raise TypeError("progress_every must be an integer and not bool")
    if progress_every <= 0:
    ...
    samples = sorted(phase7.samples, key=lambda item: item.sample_id)

    def process_sample(sample: Any) -> tuple[list[Any], list[Any]]:
        try:
        ...
        try:
            distorted_circuit = phase7.circuits_by_id[sample.distorted_circuit_id]
        except KeyError as exc:
            raise ValueError(
        ...
        return sample_candidates, sample_rollouts

    candidates: list[Any] = []
    rollouts: list[Any] = []
    started = time.monotonic()
    completed = 0
    with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="triqto-p9") as pool:
        future_by_sample = {pool.submit(process_sample, sample): sample for sample in samples}
        for future in as_completed(future_by_sample):
            sample = future_by_sample[future]
            try:
                sample_candidates, sample_rollouts = future.result()
            except Exception as exc:
                raise RuntimeError(
                    f"Phase 9 worker failed for sample {sample.sample_id}"
                ) from exc
            candidates.extend(sample_candidates)
            rollouts.extend(sample_rollouts)
            completed += 1
            if progress_callback is not None and (
                completed == len(samples) or completed % progress_every == 0
            ):
    ...
    rollout_by_action_id = _index_unique(rollouts, "action_id", "ActionRollout")
    ...
    for candidate in candidates:
        ...
        record = ActionCandidateRecordV1(
            ...
            circuit_ref=f"artifacts/circuits/{rollout.candidate_circuit_id}.qpy",
    ...
    candidate_count_by_sample = Counter(candidate.sample_id for candidate in candidates)
    summary = {
...
__all__ = ["build_action_engine_result_parallel"]

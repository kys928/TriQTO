...
def _require_nonblank_graph(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"{name} must be nonblank")
    return value


def _require_nonnegative_int_graph(value: Any, name: str) -> int:
...
def _require_safe_posix_ref_graph(value: Any, name: str) -> str:
    ...
    if ref.is_absolute() or any(part == ".." for part in ref.parts):
        raise ValueError(f"{name} must not be absolute or escape the dataset root")
    return value


@dataclass(slots=True)
class GraphRecord(ManifestRecordMixin):
    """Phase 8 graph artifact manifest row; no split or training label fields."""

    required_fields: ClassVar[tuple[str, ...]] = ("graph_id", "sample_id", "circuit_id", "role", "family", "graph_schema_version", "graph_ref", "content_hash")
    ...
    def validate(self) -> None:
        ...
        for name in self.required_fields:
            _require_nonblank_graph(getattr(self, name), name)
        if self.role not in {"clean", "distorted"}:
            raise ValueError("GraphRecord.role must be clean or distorted")
        for name in ("n_nodes", "n_edges", "n_gate_events", "node_feature_dim", "edge_feature_dim", "gate_feature_dim"):
            _require_nonnegative_int_graph(getattr(self, name), name)
        _require_safe_posix_ref_graph(self.graph_ref, "graph_ref")
...
class GraphPairRecord(ManifestRecordMixin):
    """Phase 8 clean/distorted graph pair artifact manifest row."""

    required_fields: ClassVar[tuple[str, ...]] = ("graph_pair_id", "sample_id", "clean_graph_id", "distorted_graph_id", "distortion_id", "metric_id", "pair_ref")
    ...
    def validate(self) -> None:
        ...
        for name in self.required_fields:
            _require_nonblank_graph(getattr(self, name), name)
        _require_safe_posix_ref_graph(self.pair_ref, "pair_ref")
        if not isinstance(self.metadata, Mapping):

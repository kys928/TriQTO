...
from triqto.model.constants import STREAM_ORDER, UNCERTAINTY_TARGETS
...
from .identifiability import build_identifiability_evaluation_report
...
def _item_metadata(
    ...
    distortion_type = example.metadata.get("distortion_type")
    distortion_type_value = (
        str(distortion_type)
        if isinstance(distortion_type, str) and distortion_type
        else None
    )
    backend = example.metadata.get("backend_id")
    backend_value = str(backend) if isinstance(backend, str) and backend else None
    return (
        family_value,
        raw_n_qubits,
        distortion_value,
...
def _metadata_axis_value(metadata: dict[str, Any], axis: str) -> str:
    value = metadata.get(axis)
    if axis == "n_qubits" and isinstance(value, int) and not isinstance(value, bool):
        value = str(value)
    if not isinstance(value, str) or not value:
        raise ValueError(f"Evaluation item is missing OOD axis metadata {axis}")
    return value
...
def _validate_evaluation_design(
    ...
    view_config = dataset.config
    if config.evaluation_design == "iid_test":
        if view_config.split_strategy != "clean_circuit_hash":
            raise ValueError(
                "iid_test evaluation cannot consume an axis_holdout Phase 12 dataset"
            )
        return {
            "evaluation_design": "iid_test",
            "holdout_axis": None,
            "holdout_values": [],
            "development_axis_values": [],
            "test_axis_values": [],
            "ood_axis_values_disjoint": False,
        }
    if view_config.split_strategy != "axis_holdout":
        raise ValueError(
            "OOD evaluation requires a Phase 12 axis_holdout split strategy"
        )
    if (
        view_config.holdout_axis != config.holdout_axis
        or tuple(view_config.holdout_values) != tuple(config.holdout_values)
    ):
        raise ValueError(
            "Evaluation holdout axis/values do not match the Phase 12 split contract"
        )
    assert config.holdout_axis is not None
    development_values: set[str] = set()
    test_values: set[str] = set()
    counts: Counter[str] = Counter()
    for task in config.tasks:
        for split in ("train", "validation", "test"):
            for record in dataset.records_by_task_split.get((task, split), ()):
                item = _raw_item(dataset, record.view_item_id)
                value = _metadata_axis_value(item.metadata, config.holdout_axis)
                declared_holdout = item.metadata.get("is_ood_holdout")
                if not isinstance(declared_holdout, bool):
                    raise TypeError("OOD item metadata.is_ood_holdout must be bool")
                expected_holdout = value in config.holdout_values
                if declared_holdout != expected_holdout:
                    raise ValueError("OOD item holdout marker disagrees with its axis value")
                if (split == "test") != expected_holdout:
                    raise ValueError(
                        "OOD split contains an axis value on the wrong side of the holdout"
                    )
                target = test_values if split == "test" else development_values
                target.add(value)
                counts[split] += 1
    overlap = development_values & test_values
    if overlap:
        raise ValueError(f"OOD axis values overlap development and test: {sorted(overlap)}")
    missing = set(config.holdout_values) - test_values
    if missing:
        raise ValueError(f"OOD evaluation has no test evidence for {sorted(missing)}")
    if not counts["test"] or not counts["train"]:
        raise ValueError("OOD evaluation requires nonempty train and test partitions")
    return {
        "evaluation_design": "ood_axis_holdout",
        "holdout_axis": config.holdout_axis,
        "holdout_values": list(config.holdout_values),
        "development_axis_values": sorted(development_values),
        "test_axis_values": sorted(test_values),
        "ood_axis_values_disjoint": True,
        "audited_partition_item_counts": dict(sorted(counts.items())),
    }
    ...
    *,
    ...
    born_metrics = (
        distribution_metrics_by_graph(
            ...
            distribution_index=output.born_prediction.measurement_setting_index,
        )
    ...
    hilbert_metrics = (
        distribution_metrics_by_graph(
            ...
            epsilon=config.distribution_epsilon,
            distribution_index=(
                auxiliary.born_prediction.measurement_setting_index
            ),
        )
        if (
    ...
    for graph_index, (example, raw) in enumerate(
        ...
        uncertainty_errors: dict[str, float] = {}
        ...
        identifiable_for_evaluation = (
            example.metadata.get("identifiability_status") != "unidentifiable"
        )
        if bool(diagnosis_target.class_mask[graph_index]) and identifiable_for_evaluation:
            ...
            calibration.update(
                {
                    "softmax_diagnosis_confidence": confidence,
                    "softmax_diagnosis_correct": correct,
                }
            )
            uncertainty_errors["diagnosis"] = metrics["diagnosis_nll"]
        ...
        if (
            bool(diagnosis_target.strength_mask[graph_index])
            and identifiable_for_evaluation
        ):
        ...
        if identifiable_for_evaluation and bool(node_mask.any()) and bool(
        ...
        if not identifiable_for_evaluation:
        ...
        if bool(candidate_mask.any()):
            ...
            calibration.update(
                {
                    "softmax_action_ranking_confidence": confidence,
                    "softmax_action_ranking_correct": correct,
                }
            )
            uncertainty_errors["action_ranking"] = _safe_float(
                -torch.log(local_probabilities[target_local].clamp_min(1e-12)),
                "action target nll",
            )
        ...
        if "born_kl" in born_metrics[graph_index]:
            uncertainty_errors["born_prediction"] = (
                born_metrics[graph_index]["born_kl"]
                + born_metrics[graph_index]["born_hellinger"]
            )
        if hilbert_metrics[graph_index]:
            ...
            uncertainty_errors["hilbert_to_born"] = (
                hilbert_metrics[graph_index]["born_kl"]
                + hilbert_metrics[graph_index]["born_hellinger"]
            )
        if supervised.targets.born_prediction.probabilities.numel():
            outcome_mask = (
        ...
        uncertainty_names = {
            ...
            "hilbert_to_born": "hilbert_deformation",
        }
        for task_name, realized_error in sorted(uncertainty_errors.items()):
            if not uncertainty_head_trained:
                continue
            target_name = uncertainty_names[task_name]
            target_index = UNCERTAINTY_TARGETS.index(target_name)
            log_variance = _safe_float(
                output.uncertainty.log_variance[graph_index, target_index],
                f"{task_name} uncertainty log variance",
            )
            variance = float(np.exp(log_variance))
            calibration.update(
                {
                    f"uncertainty_{task_name}_log_variance": log_variance,
                    f"uncertainty_{task_name}_variance": variance,
                    f"uncertainty_{task_name}_realized_error": realized_error,
                    f"uncertainty_{task_name}_likelihood_score": (
                        realized_error / variance + log_variance
                    ),
                }
            )
...
def run_evaluation(
    *,
    ...
    design_audit = _validate_evaluation_design(dataset, evaluation_config)
    ...
    excluded_unidentifiable_counts: Counter[str] = Counter()
    identifiability_rows_by_entity: dict[str, dict[str, Any]] = {}
    for task in evaluation_config.tasks:
        ...
        for example in task_examples:
            ...
            status = metadata.get("identifiability_status")
            if status is not None and example.entity_id not in identifiability_rows_by_entity:
                identifiability_rows_by_entity[example.entity_id] = {
                    "identifiability_status": status,
                    "identifiability_reason": metadata.get(
                        "identifiability_reason"
                    ),
                    "diagnosis_supervision_mask": bool(
                        metadata.get("diagnosis_supervision_mask", False)
                    ),
                    "metadata": {
                        "unidentifiable_supervision_override": metadata.get(
                            "unidentifiable_supervision_override",
                            False,
                        )
                    },
                }
        if task in {"diagnosis", "action_ranking"}:
            eligible = [
                example
                for example in task_examples
                if example.metadata.get("identifiability_status")
                != "unidentifiable"
            ]
            excluded_unidentifiable_counts[task] = len(task_examples) - len(eligible)
            task_examples = eligible
    ...
    identifiability_report = build_identifiability_evaluation_report(
        identifiability_rows_by_entity.values()
    ).to_dict()
            ...
            for ablation in evaluation_config.ablations:
                ...
                item_results.extend(
                    _evaluate_items_for_output(
                        ...
                        uncertainty_head_trained=(
                            training_run.training_config.loss.uncertainty_weighting
                        ),
                    )
                )
    ...
    summary = {
        ...
        "excluded_unidentifiable_task_item_counts": dict(
            sorted(excluded_unidentifiable_counts.items())
        ),
        "identifiability_evaluation": identifiability_report,
        "uncertainty_head_trained": (
            training_run.training_config.loss.uncertainty_weighting
        ),
        "uncertainty_head_evaluated": (
            training_run.training_config.loss.uncertainty_weighting
        ),

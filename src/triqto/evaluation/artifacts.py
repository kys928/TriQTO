...
def _report_payloads(result: EvaluationRunResult) -> dict[str, Any]:
    ...
    generalization = [
        value
        for value in result.aggregates
        if value.group_dimension
        in {"family", "n_qubits", "distortion_type", "backend_id"}
    ]
    calibration = [
        value
        for value in result.aggregates
    ...
    comparison_keys = sorted(
        {
            (row.task, row.baseline_name)
            for row in result.baseline_comparisons
        }
    )
    for task, name in comparison_keys:
        rows = [
            row
            for row in result.baseline_comparisons
            if row.task == task and row.baseline_name == name
        ]
        baseline_summary.setdefault(task, {})[name] = {
    ...
    return {
        "summary": {
            "claim_boundary": {
                ...
                "evaluation_design": result.config.evaluation_design,
                "ood_axis_holdout": (
                    result.config.evaluation_design == "ood_axis_holdout"
                ),
        ...
        "generalization": {
            "evaluation_design": result.config.evaluation_design,
            "claim_scope": (
                "audited_axis_holdout"
                if result.config.evaluation_design == "ood_axis_holdout"
                else "descriptive_iid_subgroups_not_ood"
            ),
            "holdout_axis": result.config.holdout_axis,
            "holdout_values": list(result.config.holdout_values),
            "rows": [asdict(value) for value in generalization],
        },
        "calibration": {
            "uncertainty_head_trained": result.summary[
                "uncertainty_head_trained"
            ],
            "uncertainty_head_evaluated": result.summary[
                "uncertainty_head_evaluated"
            ],
            "rows": [asdict(value) for value in calibration],
        },
...
def write_evaluation_dataset(
    ...
    try:
        ...
        completion = {
            ...
            "holdout_axis": result.config.holdout_axis,
            "holdout_values": list(result.config.holdout_values),
            "ood_axis_values_disjoint": result.summary[
                "ood_axis_values_disjoint"
            ],
            "uncertainty_head_trained": result.summary[
                "uncertainty_head_trained"
            ],

...
def save_evaluation_item_artifact(
    ...
    for name, value in sorted(item.arrays.items()):
        ...
        if not np.isfinite(array).all() if array.dtype.kind in {"f", "c"} else False:
    ...
    if item.content_hash and item.content_hash != content_hash:
        raise ValueError("Evaluation item content hash changed before publication")
    ...
    loaded = load_evaluation_item_artifact(target, expected_content_hash=content_hash)
...
def _report_payloads(result: EvaluationRunResult) -> dict[str, Any]:
    ...
    for name in sorted({row.baseline_name for row in result.baseline_comparisons}):
        rows = [row for row in result.baseline_comparisons if row.baseline_name == name]
        baseline_summary[name] = {
            "comparison_count": float(len(rows)),
            "learned_win_fraction": float(
                sum(row.learned_minus_baseline < 0.0 for row in rows) / len(rows)
            ),
...
def write_evaluation_dataset(
    ...
    try:
        ...
        for item in sorted(
            result.item_results,
            key=lambda value: (value.task, value.ablation, value.view_item_id),
        ):
            item.artifact_ref = (
                f"artifacts/items/{item.evaluation_item_id}.npz"
            )
        ...
        aggregate_records = [_aggregate_record(value) for value in result.aggregates]
        baseline_records = [
        ...
        writer.write_records("evaluation_item_manifest", item_records, overwrite=False)
        ...
        for name, payload in reports.items():
            write_strict_json(staging / "reports" / f"{name}.json", payload)
        ...
        for record in loaded_items:
            ...
            artifact = load_evaluation_item_artifact(
                resolve_safe_file(staging, record.artifact_ref, "evaluation artifact"),
        ...
        managed_files = sorted([*managed_before_marker, "evaluation_complete.json"])
        completion = {
            "complete": True,

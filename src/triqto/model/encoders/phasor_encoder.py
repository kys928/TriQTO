"""Periodic sine/cosine phasor encoder."""
...
class PhasorEncoder(nn.Module):
    ...
    def forward(self, batch: ParameterTensorBatch | None, graph_count: int, reference: Tensor) -> tuple[Tensor, Tensor]:
        ...
        radius_error = batch.sin.square() + batch.cos.square() - 1.0
        elements = self.element(
            torch.stack((batch.sin, batch.cos, batch.values, radius_error), dim=1)
        )
        pooled = self.pool(
            torch.cat(
                (

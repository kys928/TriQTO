...
from triqto.training.latent_extraction import LatentExtractionConfig, extract_checkpoint_latents
...
def _operational_results() -> list[object]:
    backend = local_line_backend(2)
    source = QuantumCircuit(2)
    source.h(0)
    source.cx(0, 1)
    probe = basis_probe_action(2, ("X", "Y"), circuit=source, shots=128, seed=2026)
    _, layout = layout_selection_action(source, backend, seed=2026)
    _, routing = routing_transpilation_action(source, backend, seed=2026, optimization_level=1)
    ...
    reduced.x(0)
    depth = semantics_verified_depth_reduction(reducible, reduced)
    results = [probe, layout, routing, depth]
    adapter = build_operational_action_tensor_batch(results)
...
def run(output: Path) -> dict[str, object]:
    ...
    operational_result = write_operational_action_dataset(operational, _operational_results(), source_dataset_id=generation.scientific_generation_id, evidence_tier="mixed_offline")
    ...
    latent_result = extract_checkpoint_latents(
        ...
        output_root=latent,
        config=LatentExtractionConfig(split="validation", tasks=("diagnosis",), head="diagnosis", representation="head_latent", max_points=128, batch_size=8),
        phase7_root=phase7,
    )

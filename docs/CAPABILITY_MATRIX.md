...
## Backend stream integration update

Phase 7 assigns stable local fake-backend evidence at the clean-circuit level and propagates the backend identity, coupling/basis evidence, availability masks, and missing reasons into Phase 12 diagnosis items and the Phase 14 training adapter. Backend dense features are normalized from the training split only. Calibration-like quantities that are unavailable in the local fixture remain unavailable metadata and are not encoded as observed zeros. Backend-ID OOD helpers now audit that each clean-circuit lineage keeps a single backend assignment before any backend-holdout claim is made. This is still fake/offline evidence, not hardware validation.

## Operational actions update

Standalone operational action APIs now cover basis probing, layout selection, routing/transpilation, and semantics-verified depth reduction. Each action reports explicit availability, rejection reasons, before/after metadata, and deterministic action identities. Compilation actions are not scored as privileged logical inverses, and depth reduction is accepted only when statevector semantic fidelity satisfies the declared tolerance. These APIs are executable and tested but are not yet fully wired into the Phase 9 ranking dataset as deployable policy actions.

## Latent topology update

Checkpoint-bound latent-topology diagnostics are executable only when a nonblank checkpoint identity and explicit latent point IDs/coordinates are supplied. The default comparison preserves absolute scale; optional normalized-shape diagnostics are labeled shape-only and receive distinct identities. No trained latent topology result is included in the repository, and topology loss remains exactly zero.

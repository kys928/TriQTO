...
def make_pair(graph_id_value: str):
    pair = GraphSamplePair(
        ...
        measurement_setting_ids=np.asarray(["measurement_z"], dtype="<U13"),
        measurement_basis_codes=np.asarray([[0, 0]], dtype=np.int8),
        measurement_outcome_bitstrings=np.asarray(["00", "11"], dtype="<U2"),
        measurement_setting_index=np.asarray([0, 0], dtype=np.int64),
        clean_measurement_probabilities=np.asarray([0.5, 0.5], dtype=np.float64),
        distorted_measurement_probabilities=np.asarray([0.3, 0.7], dtype=np.float64),
        ...
        observable_evidence_fingerprint="observable_test_fingerprint",
        metadata={"phase": 8},
    )

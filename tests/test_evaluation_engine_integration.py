from __future__ import annotations

import json
from pathlib import Path

from triqto.actions import (
    ActionEngineConfig,
    build_action_engine_result,
    write_action_dataset,
)
from triqto.data_generation import (
    CircuitGenerationSpec,
    DatasetGenerationConfig,
    DistortionSpec,
    generate_dataset,
    write_dataset,
)
from triqto.baselines import (
    BaselineSuiteConfig,
    run_baseline_suite,
    write_baseline_dataset,
)
from triqto.evaluation import (
    EvaluationConfig,
    run_evaluation,
    write_evaluation_dataset,
)
from triqto.graph import (
    GraphConversionConfig,
    convert_completed_dataset_to_graphs,
    snapshot_managed_files,
    write_graph_dataset,
)
from triqto.model import TriQTOModelConfig
from triqto.topology import (
    TopologyAuditConfig,
    build_topology_audit_result,
    write_topology_dataset,
)
from triqto.training import (
    CurriculumStageConfig,
    LossConfig,
    OptimizerConfig,
    SchedulerConfig,
    TrainingConfig,
    run_training,
)
from triqto.training_views import (
    TrainingViewConfig,
    assign_development_split,
    build_training_view_result,
    write_training_view_dataset,
)


def _managed_snapshot(root: Path, marker_name: str):
    marker = json.loads((root / marker_name).read_text())
    return snapshot_managed_files(root, tuple(marker["managed_files"]))


def test_phase7_to_phase15_heldout_evaluation_is_complete_and_immutable(
    tmp_path: Path,
) -> None:
    phase7_root = tmp_path / "phase7"
    graph_root = tmp_path / "phase8"
    action_root = tmp_path / "phase9"
    topology_root = tmp_path / "phase11"
    phase12_root = tmp_path / "phase12"
    phase14_root = tmp_path / "phase14"
    phase15_root = tmp_path / "phase15"
    baseline_root = tmp_path / "phase10"

    generation_config = DatasetGenerationConfig(
        dataset_name="phase15-integration-source",
        base_seed=1515,
        circuit_specs=[
            CircuitGenerationSpec(
                family="bell",
                n_qubits=2,
                generator_kwargs={"measure": True},
                repetitions=8,
            ),
            CircuitGenerationSpec(
                family="ghz",
                n_qubits=3,
                generator_kwargs={"measure": True},
                repetitions=4,
            ),
        ],
        distortion_specs=[
            DistortionSpec(
                name="rx_overrotation",
                kwargs={"strength": 0.2, "qubits": [0]},
            )
        ],
        store_statevectors=False,
        max_samples=12,
    )
    generation_result = generate_dataset(generation_config)
    write_dataset(generation_result, phase7_root)

    graph_result = convert_completed_dataset_to_graphs(
        phase7_root,
        GraphConversionConfig(include_supplemental_counts=False),
    )
    write_graph_dataset(graph_result, graph_root)

    action_result = build_action_engine_result(
        phase7_root,
        graph_root,
        ActionEngineConfig(
            candidate_magnitudes=(0.2,),
            max_candidates_per_sample=64,
            max_edits_per_action=16,
        ),
    )
    write_action_dataset(action_result, action_root)
    baseline_result = run_baseline_suite(
        phase7_root,
        graph_root,
        action_root,
        BaselineSuiteConfig(
            enabled_baselines=("random_correction",),
            random_seed=1515,
            max_optimizer_dimensions=32,
            max_objective_evaluations=100,
        ),
    )
    write_baseline_dataset(baseline_result, baseline_root)

    topology_result = build_topology_audit_result(
        phase7_root,
        graph_root,
        action_root,
        TopologyAuditConfig(
            min_points=3,
            betti_grid_size=8,
            top_k_lifetimes=2,
            max_points_per_group=128,
            max_groups=64,
            max_statevector_amplitudes=64,
        ),
    )
    write_topology_dataset(topology_result, topology_root)

    clean_circuit_ids = sorted(
        {
            sample.clean_circuit_id
            for sample in generation_result.samples
            if sample.family == "bell"
        }
    )
    assert len(clean_circuit_ids) >= 2
    view_config = None
    for split_seed in range(20_000):
        candidate = TrainingViewConfig(
            tasks=(
                "action_ranking",
                "born_prediction",
                "joint_multitask",
                "hardware_masked",
            ),
            split_strategy="axis_holdout",
            holdout_axis="family",
            holdout_values=("ghz",),
            split_seed=split_seed,
            train_fraction=0.75,
            validation_fraction=0.25,
            test_fraction=0.0,
            max_items=1000,
            max_candidates_per_item=128,
            max_source_refs_per_item=1024,
        )
        assigned = {
            assign_development_split(circuit_id, candidate)
            for circuit_id in clean_circuit_ids
        }
        if assigned == {"train", "validation"}:
            view_config = candidate
            break
    assert view_config is not None

    view_result = build_training_view_result(
        phase7_root,
        graph_root,
        action_root,
        topology_root,
        view_config,
    )
    write_training_view_dataset(view_result, phase12_root)

    training_config = TrainingConfig(
        run_name="phase15_integration_training",
        seed=1515,
        stages=(
            CurriculumStageConfig(
                name="born_only",
                epochs=1,
                tasks=(
                    "action_ranking",
                    "born_prediction",
                    "joint_multitask",
                    "hardware_masked",
                ),
            ),
        ),
        batch_size=2,
        optimizer=OptimizerConfig(
            name="adamw",
            learning_rate=1e-3,
            weight_decay=0.0,
        ),
        scheduler=SchedulerConfig(
            name="constant",
            warmup_steps=0,
            minimum_learning_rate_ratio=1.0,
        ),
        loss=LossConfig(
            geometry_weight=0.0,
            uncertainty_weighting=True,
        ),
        deterministic_algorithms=True,
        device="cpu",
        checkpoint_every_epochs=1,
        early_stopping_patience=0,
        topology_loss_weight=0.0,
    )
    model_config = TriQTOModelConfig(
        hidden_dim=32,
        graph_message_passing_layers=1,
        residual_mlp_layers=1,
        backend_input_dim=4,
        topology_input_dim=64,
        hilbert_deformation_dim=8,
        topology_prediction_dim=8,
        dropout=0.0,
        initialization_seed=1515,
    )
    run_training(
        training_view_root=phase12_root,
        output_root=phase14_root,
        training_config=training_config,
        model_config=model_config,
        phase7_root=phase7_root,
    )

    phase7_before = _managed_snapshot(phase7_root, "dataset_complete.json")
    phase12_before = _managed_snapshot(
        phase12_root,
        "training_view_complete.json",
    )
    phase14_before = _managed_snapshot(
        phase14_root,
        "training_complete.json",
    )

    evaluation_config = EvaluationConfig(
        run_name="phase15_integration_evaluation",
        seed=1515,
        tasks=(
            "action_ranking",
            "born_prediction",
            "joint_multitask",
            "hardware_masked",
        ),
        ablations=("full", "no_topology", "no_hilbert"),
        batch_size=2,
        calibration_bins=5,
        checkpoint_selection="best",
        require_test_items=True,
        include_baseline_comparison=True,
        max_items=1000,
        device="cpu",
        evaluation_design="ood_axis_holdout",
        holdout_axis="family",
        holdout_values=("ghz",),
    )
    evaluation_result = run_evaluation(
        training_view_root=phase12_root,
        training_run_root=phase14_root,
        output_root=phase15_root,
        evaluation_config=evaluation_config,
        phase7_root=phase7_root,
        graph_root=graph_root,
        action_root=action_root,
        baseline_root=baseline_root,
    )
    write_evaluation_dataset(evaluation_result, phase15_root)

    marker = json.loads((phase15_root / "evaluation_complete.json").read_text())
    summary = json.loads((phase15_root / "evaluation_summary.json").read_text())
    assert marker["complete"] is True
    assert marker["heldout_split"] == "test"
    assert marker["evaluation_design"] == "ood_axis_holdout"
    assert marker["holdout_axis"] == "family"
    assert marker["holdout_values"] == ["ghz"]
    assert marker["training_performed"] is False
    assert marker["hardware_execution_performed"] is False
    assert marker["topology_loss_weight"] == 0.0
    assert summary["heldout_evaluation_performed"] is True
    assert summary["evaluation_design"] == "ood_axis_holdout"
    assert summary["holdout_axis"] == "family"
    assert summary["development_axis_values"] == ["bell"]
    assert summary["test_axis_values"] == ["ghz"]
    assert summary["ood_axis_values_disjoint"] is True
    assert summary["uncertainty_head_trained"] is True
    assert summary["uncertainty_head_evaluated"] is True
    assert summary["baseline_comparison_count"] > 0
    assert len(
        {
            row.evaluation_baseline_id
            for row in evaluation_result.baseline_comparisons
        }
    ) == len(evaluation_result.baseline_comparisons)
    assert {
        row.task for row in evaluation_result.baseline_comparisons
    } == {"action_ranking", "joint_multitask", "hardware_masked"}
    baseline_report = json.loads(
        (phase15_root / "reports" / "baselines.json").read_text()
    )
    assert set(baseline_report) == {
        "action_ranking",
        "hardware_masked",
        "joint_multitask",
    }
    generalization_report = json.loads(
        (phase15_root / "reports" / "generalization.json").read_text()
    )
    assert generalization_report["claim_scope"] == "audited_axis_holdout"
    assert generalization_report["holdout_axis"] == "family"
    assert any(
        any(name.startswith("uncertainty_") for name in item.calibration)
        for item in evaluation_result.item_results
    )
    assert summary["test_split_item_count"] > 0
    assert summary["evaluation_item_result_count"] == (
        summary["test_split_item_count"] * 3
    )
    assert (phase15_root / "reports" / "generalization.json").is_file()
    assert (phase15_root / "reports" / "ablations.json").is_file()

    assert _managed_snapshot(phase7_root, "dataset_complete.json") == phase7_before
    assert _managed_snapshot(
        phase12_root,
        "training_view_complete.json",
    ) == phase12_before
    assert _managed_snapshot(
        phase14_root,
        "training_complete.json",
    ) == phase14_before

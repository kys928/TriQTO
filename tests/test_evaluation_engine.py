...
def test_evaluation_config_is_strict_and_roundtrips() -> None:
    ...
    ood = EvaluationConfig(
        tasks=("diagnosis",),
        evaluation_design="ood_axis_holdout",
        holdout_axis="family",
        holdout_values=("ghz",),
    )
    assert ood.holdout_values == ("ghz",)
    with pytest.raises(ValueError, match="requires holdout_axis"):
        EvaluationConfig(evaluation_design="ood_axis_holdout")


def test_repository_ood_configs_are_strict_and_axis_matched() -> None:
    from triqto.evaluation import load_evaluation_config
    from triqto.training_views import load_training_view_config

    root = Path(__file__).resolve().parents[1]
    pairs = (
        ("heldout_family_ghz.yaml", "heldout_families.yaml", "family"),
        ("heldout_n_qubits_4.yaml", "heldout_qubit_counts.yaml", "n_qubits"),
        (
            "heldout_distortion_rx_overrotation.yaml",
            "heldout_distortions.yaml",
            "distortion_type",
        ),
    )
    for training_name, evaluation_name, axis in pairs:
        training = load_training_view_config(
            root / "configs" / "training_views" / training_name
        )
        evaluation = load_evaluation_config(
            root / "configs" / "eval" / evaluation_name
        )
        assert training.split_strategy == "axis_holdout"
        assert evaluation.evaluation_design == "ood_axis_holdout"
        assert training.holdout_axis == evaluation.holdout_axis == axis
        assert training.holdout_values == evaluation.holdout_values
...
def test_distribution_metrics_do_not_mix_measurement_settings() -> None:
    rows = distribution_metrics_by_graph(
        torch.tensor([1.0, 0.0, 0.5, 0.5], dtype=torch.float64),
        torch.tensor([1.0, 0.0, 1.0, 0.0], dtype=torch.float64),
        torch.ones(4, dtype=torch.bool),
        torch.zeros(4, dtype=torch.long),
        1,
        epsilon=1e-12,
        distribution_index=torch.tensor([0, 0, 1, 1], dtype=torch.long),
    )
    assert rows[0]["born_kl"] == pytest.approx(np.log(2.0) / 2.0)
    assert rows[0]["born_total_variation"] == pytest.approx(0.25)
...
def test_calibration_metrics_are_transparent() -> None:
    ...
    assert direct["variance_calibration_error"] >= 0.0
    assert direct["occupied_variance_bins"] == 2.0
